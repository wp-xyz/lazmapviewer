...
begin
  ...
  begin
    ...
    if vArea.Intersection(ObjArea, Area) then
...
var
  ...
  lst: TGPSObjList;
begin
  Area.Create(Engine.ScreenToLonLat(Point(aLeft, aTop)), Engine.ScreenToLonLat(Point(aRight, aBottom)));
...
begin
  ...
  // Define area of +/-ATolerance pixels around the screen point (where ATolerance defaults to DELTA)
  rArea.Create(ScreenToLonLat(Point(X-ATolerance, Y-ATolerance)), ScreenToLonLat(Point(X+ATolerance, Y+ATolerance)));
...
var
  ...
  tl, br: TRealPoint;
begin
  tl := Engine.ScreenToLonLat(Point(0, 0));
  br := Engine.ScreenToLonLat(Point(Width, Height));
